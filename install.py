...
def configure_git():
    ...
    call('git config --global user.name "Wojtek Matyskiewicz"', shell=True)
    call('git config --global user.email "wojtek@matyskiewicz.com"', shell=True)
    call('git config --global core.editor "subl -n -w"', shell=True)
    call('git config --global core.excludesfile "~/.dot_files/git/.gitignore_global"', shell=True)
